...
for i in range (0, n_clusters):
    plt.scatter(T[i*num_samples:i*num_samples+num_samples, 0], T[i*num_samples:i*num_samples+num_samples, 1],
                c=colors[i], marker='.',
                alpha=0.4, 
...
plt.legend ()
plt.savefig ('gt_' + str (n_clusters) + '_' + str (dim_method)  + '_' + str (num_samples) + '.png')
...
for i in range (0, n_clusters):
    ...
    centroid_dim[i][1], 'class ' + str (i), 
    color='black', fontsize=14)

plt.title ('Graphite: ' + str (n_clusters) + ' classe(s) - unsupervised')
plt.xlabel (dim_method + ' 1')
plt.ylabel (dim_method + ' 2')
plt.savefig ('clustering_' + str (n_clusters) + '_' + str (dim_method) + '_' + str (num_samples) + '.png')
